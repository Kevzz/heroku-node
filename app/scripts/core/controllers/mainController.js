...
.controller('UserCtrl', function ($scope, $http, $window, apiService) {
  $scope.user = {username: 'john.doe', password: 'foobar'};
  $scope.isAuthenticated = false;
  $scope.welcome = '';
  $scope.message = '';

  $scope.url_base64_decode=function(str) {
  ...
  $scope.submit = function () {
    $http
      ...
      .success(function (data, status, headers, config) {
        ...
        var profile = JSON.parse($scope.url_base64_decode(encodedProfile));
